...
int main(void)
{
  ...
  MX_NVIC_Init();
  /* USER CODE BEGIN 2 */
  hal_uart_USART1_init ();
  app_modbus_init ();


  HAL_UART_Receive_IT(&huart1, rx_buff, 10);
  HAL_UART_Receive_IT(&huart2, rx_buff2, 10);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  for ( volatile uint32_t u32_cpt=0 ; u32_cpt<0x1f ; u32_cpt++){}
	  app_modbus_autom ();
...
static void MX_USART1_UART_Init(void)
{
  ...
  huart1.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_TXINVERT_INIT|UART_ADVFEATURE_RXOVERRUNDISABLE_INIT
